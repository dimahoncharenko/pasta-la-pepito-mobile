...
import type { ApiDishResponse } from "./dishApi.types"
...
export class DishApi {
  ...
  async getDishes(): Promise<{ kind: "ok"; dishes: Dish[] } | GeneralApiProblem> {
    const response: ApiResponse<ApiDishResponse[]> = await this.apisauce.get("/dish")
  ...
  async getNewDishes(): Promise<{ kind: "ok"; hits: Dish[] } | GeneralApiProblem> {
    const response: ApiResponse<{ hits: ApiDishResponse[] }> = await this.apisauce.get(
      "/dish/hits-and-news",
    )
